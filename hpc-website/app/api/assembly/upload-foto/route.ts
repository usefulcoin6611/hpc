...
export async function POST(request: NextRequest) {
  try {
    ...
    const uploadDir = join(process.cwd(), 'public', 'uploads', 'assembly', noSeri)
    ...
    const fileName = `assembly_${noSeri}_${timestamp}.${fileExtension}`
    ...
    const fileUrl = `/uploads/assembly/${noSeri}/${fileName}`

    // Simpan data foto ke database
    const fotoAssembly = await (prisma as any).fotoAssembly.create({
      data: {
        fileName,
        fileUrl,
        fileSize: file.size,
    ...
    return NextResponse.json({
      ...
      data: {
        id: fotoAssembly.id,
        fileName: fotoAssembly.fileName,
        fileUrl: fotoAssembly.fileUrl,
        fileSize: fotoAssembly.fileSize,
        fileType: fotoAssembly.fileType,
        keterangan: fotoAssembly.keterangan,
        uploadDate: fotoAssembly.createdAt
      }
    })
...
export async function GET(request: NextRequest) {
  try {
    ...
    const fotoList = await (prisma as any).fotoAssembly.findMany({
      where: {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Ambil data foto dari database
    const foto = await (prisma as any).fotoAssembly.findFirst({
    ...
    await (prisma as any).fotoAssembly.update({
