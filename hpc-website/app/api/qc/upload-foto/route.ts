...
export async function POST(request: NextRequest) {
  try {
    ...
    const fileName = `qc_${noSeri}_${timestamp}${fileExtension}`
    ...
    // Save to database
    const fotoQC = await (prisma as any).fotoQC.create({
      data: {
        fileName: fileName,
        fileUrl: `/uploads/qc/${fileName}`,
        fileSize: file.size,
        fileType: file.type,
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get foto list
    const fotoList = await (prisma as any).fotoQC.findMany({
      where: {
        detailBarangMasukNoSeriId: detailNoSeri.id,
        isActive: true
      },
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    // Get foto data
    const foto = await (prisma as any).fotoQC.findUnique({
      where: { id: fotoId }
    })
    ...
    await (prisma as any).fotoQC.update({
      where: { id: fotoId },
      data: { isActive: false }
    })
